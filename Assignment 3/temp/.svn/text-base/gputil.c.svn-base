...
// 			if(tempTok) free(tempTok);
			...
			if(tempTok==NULL) {
				...
				if(strlen(tempTok) > 1){
fprintf(stderr, "length of tok is %d and token is %s\n", (int)strlen(tempTok), tempTok);
					errCode = VALUE;
					errFlag = true;
D
				}
...
fprintf(stderr, "tempTok symbol is %s\n", tempTok);
			if(tempTok==NULL) {
				errCode = FIELD;
				errFlag = true;
			}
...
fprintf(stderr, "len of symbol is %d\n", (int)strlen(filep->waypt[i].symbol));
...
// 		fprintf(stderr, "filename is %s\n", filename);
		...
		if(filename!=NULL) {
			...
			if(fileStatus.code!=0) {
				...
				fprintf(stderr, "Error reading file. Error returned '%s' in line '%d'\n", errCode, fileStatus.lineno);
