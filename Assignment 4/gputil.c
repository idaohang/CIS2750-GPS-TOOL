...
PyObject *Gps_readFile(PyObject *self, PyObject *args);
PyObject *Gps_getData( PyObject *self, PyObject *args );
PyObject *Gps_freeFile( PyObject *self, PyObject *args );

static PyMethodDef gpsMethods[] = { {"readFile", Gps_readFile, METH_VARARGS}, 
				    {"getData", Gps_getData, METH_VARARGS}, 
				    {"freeFile", Gps_freeFile, METH_NOARGS}, {NULL, NULL} };

void initGps( void ) { Py_InitModule("Gps", gpsMethods); }
	...
	if(filep->nwaypts > 0) {
		...
		if(strlen(filep->waypt[0].symbol)!=0) {
			...
			if(SymLen!=0) {
				...
				while(strlen(Symstr) != SymLen+3) {
					strcat(Symstr, "-");
// 			 		fprintf(stderr, "Symstr is %s and strlen is %d\n", Symstr, (int)strlen(Symstr));
				}
			}
		}
		...
		fprintf(gpf, "F %s Latitude   Longitude%s   T O%s\n", IDstr, Symstr, cmtstr); line++;
...
// 				filep->waypt[i].coord.lon = filep->waypt[i].coord.lon * (-1);
			...
			fprintf(gpf, "W %s %09.6lf %010.6lf ", filep->waypt[i].ID, filep->waypt[i].coord.lat, filep->waypt[i].coord.lon);
...
// 				filep->trkpt[i].coord.lon = filep->trkpt[i].coord.lon * (-1);
			...
			fprintf(gpf, "T %09.6lf %010.6lf ", filep->trkpt[i].coord.lat, filep->trkpt[i].coord.lon);
...
static GpTrack* fileTrack;
static GpFile* filep;
static int trackno;

/*
	Function: Gps_readFile
	Pre-Condition: Tuples passed from Python which contains the filename
	Post-Condition: Tuples passed back to Python inidicating file read status and message
	Comment: One of the necessary wrapper functions written so Python can communication with C functions
*/
PyObject *Gps_readFile(PyObject *self, PyObject *args) {
...
// 		fprintf(stderr, "filename is %s\n", filename);
		...
		if(filename!=NULL) {
			...
			if(inputFile==NULL) {
				...
				return Py_BuildValue("ss", "ERROR", "Error opening file");
			}
	
			filep = malloc(sizeof(GpFile));
			assert(filep);
	
			fileStatus = readGpFile(inputFile, filep);
			...
			if(fileStatus.code!=0) {
				...
				switch(fileStatus.code) {
					...
					case 0: strcpy(errCode, "OK");
						break;
					case 1: strcpy(errCode,"IOERR");  // I/O error
						break;
					case 2: strcpy(errCode,"UNKREC"); // unknown record type
						break;
					case 3: strcpy(errCode,"BADSEP"); // bad field separator, only blank allowed
						break;
					case 4: strcpy(errCode,"FILTYP"); // unacceptable file type
						break;
					case 5: strcpy(errCode,"DATUM");  // unacceptable datum
						break;
					case 6: strcpy(errCode,"COORD");  // coordinates in unacceptable format
						break;
					case 7: strcpy(errCode,"NOFORM"); // no 'F' format record prior to data records
						break;
					case 8: strcpy(errCode,"FIELD");  // unknown field, or required field missing
						break;
					case 9: strcpy(errCode,"VALUE");  // a field had an invalid or out-of-range value
						break;
					case 10: strcpy(errCode, "DUPRT");  // duplicate route no.
						break;
					case 11: strcpy(errCode,"UNKWPT"); // unknown waypoint ID
						break;
	
				}
	
				fprintf(stderr, "Error reading file. Error returned '%s' on line '%d'\n", errCode, fileStatus.lineno);
				return Py_BuildValue("ss", "ERROR", "Error reading file");
			}
	
			trackno = getGpTracks(filep, &fileTrack);
		}
	
		else {
			fprintf(stderr, "Error: Input filename is invalid.\n");
			return Py_BuildValue("ss", "ERROR", "Input file name is invalid");
		}
...
/*
	...
	Pre-Condition: Tuples passed from Python which contains the the address of the 4 point values which are initialized in Python
	Post-Condition: Tuples passed back to Python inidicating file read status and message. The 4 points are also filledo ut by the function
	Comment: One of the necessary wrapper functions written so Python can communication with C functions. 
		 Calling this function will allow the C functions to fill out the necessary Data needed by Python. These are passed back by reference
*/
PyObject *Gps_getData( PyObject *self, PyObject *args ) {
	...
	PyObject *waypts, *routes, *tracks, *trkpts;

	PyArg_ParseTuple(args, "OOOO", &waypts, &routes, &trkpts, &tracks);

	for(i=0; i<filep->nwaypts; i++) {
		PyList_Append(waypts, Py_BuildValue("ffss", filep->waypt[i].coord.lat, filep->waypt[i].coord.lon, filep->waypt[i].ID, filep->waypt[i].comment));
	}

	for(i=0; i<filep->nroutes; i++) {
		PyObject *routeList = PyList_New(filep->route[i]->npoints + 2);
		PyList_SET_ITEM(routeList, 0, Py_BuildValue("i", filep->route[i]->number));
		PyList_SET_ITEM(routeList, 1, Py_BuildValue("s", filep->route[i]->comment));
		
		for(m=2, j=0; j<filep->route[i]->npoints; j++) {
			PyList_SET_ITEM(routeList, m+j, Py_BuildValue("i", filep->route[i]->leg[j]));
		}

		PyList_Append(routes, routeList);
	}

	for(i=0; i<filep->ntrkpts; i++) {
		PyList_Append(trkpts, Py_BuildValue("ff", filep->trkpt[i].coord.lat, filep->trkpt[i].coord.lon));
	}

	for(i=0; i<trackno; i++) {
		strcpy(allTime, filep->dateFormat);
		strcat(allTime, " %H:%M:%S");
		trackTime = localtime(&(fileTrack)[i].startTrk);
		strftime (buffer, 128, allTime, trackTime);
		PyList_Append(tracks, Py_BuildValue("is", fileTrack[i].seqno, buffer));
	}
...
/*
	Function: Gps_freeFile
	Pre-Condition: none
	Post-Condition: Tuples passed back to Python inidicating file free status and message
	Comment: One of the necessary wrapper functions written so Python can communication with C functions. 
		 This will free the GpFile opened by previous functions and is called from Python
*/
PyObject *Gps_freeFile( PyObject *self, PyObject *args ) {

	freeGpFile(filep);

	if(fileTrack) free(fileTrack);
